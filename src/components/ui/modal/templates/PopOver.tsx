...
import { Info, MessageCircleWarning, X } from "lucide-react";
...
export function PopOver({ message, type = "INFORMATION", functionAfterComplete, id }: PopOver) {
    ...
    const background = type === "INFORMATION" ? "bg-green-500" : type === "WARNING" ? "bg-orange-500/70" : "bg-red-800/70";
    const icon = type === "INFORMATION" ? <Info className="text-3xl" /> : type === "WARNING" ? <MessageCircleWarning className="text-3xl" /> : <X className="text-3xl" />
    ...
    return (
        <div
            className="w-screen h-screen text-white flex justify-end items-start md:mt-12 animate-jump-screen z-[999]"
            ref={contentRef}
